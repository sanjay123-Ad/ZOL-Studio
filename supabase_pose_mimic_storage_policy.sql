-- =====================================================
-- POSE MIMIC STORAGE BUCKET SETUP
-- =====================================================
-- This SQL script creates the storage bucket and policies
-- for the AI Pose Mimic feature.
...
-- File structure: pose_mimic/{userId}/pose-{fileId}-{timestamp}.png
-- This ensures complete isolation between users.
-- =====================================================

-- Drop existing policies if they exist (to avoid conflicts)
DROP POLICY IF EXISTS "Pose Mimic: Users can upload to their own folder" ON storage.objects;
DROP POLICY IF EXISTS "Pose Mimic: Users can read their own files" ON storage.objects;
DROP POLICY IF EXISTS "Pose Mimic: Users can update their own files" ON storage.objects;
DROP POLICY IF EXISTS "Pose Mimic: Users can delete their own files" ON storage.objects;

-- Policy 1: Users can ONLY upload files to their own folder
-- The folder name must match their user ID (auth.uid())
CREATE POLICY "Pose Mimic: Users can upload to their own folder"
ON storage.objects
FOR INSERT
TO authenticated
WITH CHECK (
  bucket_id = 'pose_mimic' 
  AND (storage.foldername(name))[1] = auth.uid()::text
);

-- Policy 2: Users can ONLY read files from their own folder
-- Prevents users from accessing other users' files
CREATE POLICY "Pose Mimic: Users can read their own files"
ON storage.objects
FOR SELECT
TO authenticated
USING (
...
CREATE POLICY "Pose Mimic: Users can update their own files"
ON storage.objects
FOR UPDATE
TO authenticated
USING (
...
CREATE POLICY "Pose Mimic: Users can delete their own files"
ON storage.objects
FOR DELETE
TO authenticated
USING (
...
-- 2. You should see 'pose_mimic' bucket
