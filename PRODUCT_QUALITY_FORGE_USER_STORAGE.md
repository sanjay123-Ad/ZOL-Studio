# Product Quality Forge - User-Based Storage Structure

## ✅ User Isolation Guarantee

The Product Quality Forge storage is **completely separated by user**. Each user's files are stored in their own dedicated folder, and policies ensure users can **ONLY** access their own files.
...
product_quality_forge/
  ...
  │   └── forge-file3-1234567892.png
  │
  ├── {user-id-2}/                    ← User 2's dedicated folder
  │   ├── forge-file1-1234567893.png
  │   └── forge-file2-1234567894.png
  │
  └── {user-id-3}/                    ← User 3's dedicated folder
      └── forge-file1-1234567895.png
```
...
1. **File Path Structure:**
   - Format: `{userId}/forge-{fileId}-{timestamp}.png`
   - The first folder level is **always** the user's ID
   - Example: `bf02cc68-ba0e-4dc4-932a-094795f73aa9/forge-abc123-1699123456789.png`
...
1. Go to **Storage** → `product_quality_forge` bucket
