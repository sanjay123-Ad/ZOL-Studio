...
export async function generateProductQualityForge(
    ...
    try {
        ...
        if (garmentType === 'lower') {
            console.log('[Product Quality Forge] Analyzing pant image for detailed recreation...');
            pantAnalysis = await analyzePantImage(sourceImage);
            console.log('[Product Quality Forge] Pant analysis completed:', pantAnalysis.substring(0, 200) + '...');
        }
...
- **Fabric Type:** Do NOT change the fabric type - preserve denim, cotton, twill, etc. from IMAGE 1`
            : `## UPPER GARMENT (T-SHIRT/TOP) SPECIFIC REQUIREMENTS:
...
- **DISPLAY STYLE:** Match the hanging/display method from IMAGE 2 while preserving IMAGE 1's garment structure.

- **DETAIL PRESERVATION:** Lock collar design, sleeve cuffs, side seams, and any decorative elements from IMAGE 1.`;

        const prompt = `# TASK: SUPREME 4K PRODUCT FORGE (IDENTITY SUPREMACY)
...
- **CRITICAL REQUIREMENT:** ${garmentType === 'lower' 
    ...
    : `The garment in the output MUST be a 100% exact replica of IMAGE 1 (logos, exact color shades, unique patterns, physical shape, and ALL structural details).`}
...
## 2. QUALITY BluePRINT (IMAGE 2)

- **ENVIRONMENT:** Mimic the professional studio background, floor texture, and hanger/display style from IMAGE 2 EXACTLY.

- **LIGHTING:** Replicate the high-end commercial "High-Key" studio lighting patterns (rim lights, soft fill, 3D depth, shadow placement) from IMAGE 2 with precision.

- **COMPOSITION:** Match the camera angle, perspective, and framing style from IMAGE 2.
...
- The output must look like a professional product photography session of IMAGE 1's garment in IMAGE 2's studio environment.`;
