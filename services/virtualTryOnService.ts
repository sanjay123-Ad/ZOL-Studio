...
**Aspect Ratio (CRITICAL - STRICT REQUIREMENT):** The final image canvas MUST have an aspect ratio of EXACTLY ${aspectRatio}. This is non-negotiable. The output image dimensions must strictly match this ratio. For example:
- If ${aspectRatio} is '1:1', the image must be perfectly square (e.g., 1024x1024, 2048x2048).
- If ${aspectRatio} is '4:5', the image must have a 4:5 ratio (e.g., 1024x1280, 2048x2560).
- If ${aspectRatio} is '9:16', the image must have a 9:16 ratio (e.g., 1080x1920, 1440x2560). The width must be 9 units and height must be 16 units - this is a vertical portrait orientation.
            ...
            for (const part of response.candidates[0].content.parts) {
                if (part.inlineData) {
                    ...
                    if (userId) {
                        ...
                        // Log usage asynchronously to speed up response
                        logUsage(userId, 'Virtual Photoshoot', 2, promptLength, 1).catch(console.error);
                    }
                    return result;
                }
            }
            ...
            for (const part of response.candidates[0].content.parts) {
                if (part.inlineData) {
                    ...
                    if (userId) {
                        ...
                        logUsage(userId, 'Refine Image', 1, promptLength, 1).catch(console.error);
                    }
                    return result;
                }
            }
...
Do not provide any explanation, just the JSON object.`;
        ...
        if (userId) {
            const promptLength = prompt.length;
            // Log usage asynchronously
            logUsage(userId, 'Diagnose Image', 3, promptLength, 0).catch(console.error);
        }
    }
}
