    ...
    try {
        ...
        if (gender === 'Male') {
            if (garmentType === 'upper') {
                if (!handType) throw new Error('Hand type must be provided for male upper body garments.');
                mannequinUrl = MANNEQUIN_MODELS.male.upper[handType][view];
