...
const UPPER_REFERENCE_URL = 'https://i.postimg.cc/XYPWLNzq/Gemini-Generated-Image-fy7c66fy7c66fy7c.png'; // T-shirt reference
const LOWER_REFERENCE_URL = 'https://i.postimg.cc/59k4K2Cq/pant-model.png'; // Pant reference
...
const CatalogForgedPage: React.FC<CatalogForgedPageProps> = ({ user }) => {
  ...
  const [hasApiKey, setHasApiKey] = useState<boolean>(true); // Assume API key is set via env
            ...
            for (const fileId of savedState.sourceFileIds) {
              ...
              if (imageData) {
                ...
                loadedFiles.push({
                  ...
                  status: savedState.fileStatuses?.[fileId] || 'queued',
  ...
  useEffect(() => {
    ...
    saveCurrentState();
  }, [isStateLoading, sourceFiles, hasStarted, garmentType, user.id, SESSION_STORAGE_KEY, SOURCE_FILES_KEY]);
  ...
  const dataUrlToImageFile = (dataUrl: string): ImageFile | null => {
    try {
      ...
      console.error("Failed to convert data URL to ImageFile", error);
      return null;
    }
  };
  ...
  const handleImageError = async (fileId: string, imagePath: string | undefined, imgElement: HTMLImageElement) => {
    ...
    if (refreshed) {
      const updatedFile = sourceFiles.find(f => f.id === fileId);
  ...
  const handleGenerate = async () => {
    ...
    const totalCreditsNeeded = sourceFiles.length * CREDITS_PER_IMAGE;
    ...
    try {
      // Fetch the internal high-fidelity style reference based on garment type
      const referenceUrl = garmentType === 'upper' ? UPPER_REFERENCE_URL : LOWER_REFERENCE_URL;
      
      // Debug: Log which reference URL is being used
      console.log(`[Product Quality Forge] Garment Type: ${garmentType}, Using Reference URL: ${referenceUrl}`);
      ...
      // Process with controlled parallelism (2-3 concurrent requests) for better speed
      const CONCURRENT_LIMIT = 2; // Process 2 images at a time
      ...
      for (let i = 0; i < filesToProcess.length; i += CONCURRENT_LIMIT) {
        ...
        await Promise.all(batch.map(async (f) => {
          ...
          try {
            ...
            await deductCredits(user.id, CREDITS_PER_IMAGE, 'product_quality_forge');
            ...
            setSourceFiles(prev => {
              const updated = prev.map(item => item.id === f.id ? { 
                  ...
                  generatedPath: imagePath // Store path for later reference
              } : item);
              
              // Save the path to state immediately
            ...
            console.error(err);
            const msg = (err as Error).message || 'Forge failed.';
            setSourceFiles(prev => prev.map(item => item.id === f.id ? { ...item, status: 'error', error: msg } : item));
          }
        }));
      }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center p-4 sm:p-6 lg:p-8 bg-gradient-to-b from-sky-50 via-white to-white dark:from-gray-900 dark:via-gray-900 dark:to-gray-900 transition-colors duration-200">
      <main className="w-full max-w-7xl">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 mb-2 bg-sky-600 text-white px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest">
            ...
            Gemini 3 Pro Workspace
          </div>
          <h1 className="text-4xl sm:text-5xl font-bold text-gray-900 dark:text-white font-headline">Product Quality Forge</h1>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  ...
                  {garmentType === 'upper' && <span className="ml-2 text-sky-600 font-semibold">• Using T-Shirt Reference</span>}
                  {garmentType === 'lower' && <span className="ml-2 text-sky-600 font-semibold">• Using Pant Reference</span>}
                </p>
              </div>
              <div className="flex items-center gap-3">
                <button
                  onClick={() => {
                    ...
                    // Clear any cached reference when switching
          ...
          <div className="flex flex-col h-full">
            <div className="flex items-center justify-between mb-3 px-2">
                ...
                {sourceFiles.length > 0 && (
                    <button onClick={() => setSourceFiles([])} className="text-[10px] font-bold text-sky-600 dark:text-sky-400 hover:underline uppercase">Clear All</button>
                )}
            </div>
            
            <div 
                onClick={() => sourceFiles.length < BATCH_LIMIT && fileInputRef.current?.click()}
                className={`relative min-h-[500px] border-2 border-dashed rounded-[32px] transition-all duration-300 overflow-hidden ${
                    sourceFiles.length === 0 ? 'bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600 hover:border-sky-600 dark:hover:border-sky-400 cursor-pointer' : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700'
                }`}
            >
                ...
                {sourceFiles.length === 0 ? (
                    <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-8 pointer-events-none">
                        ...
                        <div className="mt-8 px-6 py-2 bg-sky-50 dark:bg-sky-900/30 rounded-full text-xs font-semibold text-sky-600 dark:text-sky-400">
          ...
          <div className="animate-fade-in-up pt-10 border-t border-gray-200 dark:border-gray-700">
             <div className="flex flex-wrap justify-between items-end gap-4 mb-10">
                <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 uppercase tracking-widest font-bold">Applying Supreme Identity Lock via Gemini 3 Pro</p>
                </div>
                <button 
      ...
      <footer className="text-center py-12 text-gray-400 dark:text-gray-500 text-[10px] uppercase tracking-[0.2em]"><p>&copy; {new Date().getFullYear()} ZOLA AI Fashion Studio. Powered by Gemini 3 Pro.</p></footer>
