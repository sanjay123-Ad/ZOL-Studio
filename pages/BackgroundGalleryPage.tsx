...
const BackgroundGalleryPage: React.FC<BackgroundGalleryPageProps> = ({ onBack }) => {
  ...
  const handleApplyBackground = async (background: Background) => {
    ...
    // Check and deduct credits before background generation
    ...
    try {
        ...
        const resultBase64 = await changeBackgroundImage(subjectImageFile, backgroundImageFile, userId);
        const newImageUrl = await uploadStyleSceneImage(userId, poseId, resultBase64);

        // Deduct credit only after successful generation
        const creditResult = await deductCredits(userId, 1, 'style_scene_background');
        ...
        <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100 dark:bg-gray-900 p-4 transition-colors duration-200">
            {error ? (
                <div className="max-w-md w-full bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8 text-center transition-colors duration-200">
                    ...
                    <button 
                        onClick={onBack} 
                        className="px-6 py-2.5 bg-sky-600 text-white font-semibold rounded-full shadow-lg hover:bg-sky-700 transition-colors"
                    >
    ...
    <div className="min-h-screen flex flex-col bg-gray-50 dark:bg-gray-900 text-[#2E1E1E] dark:text-white transition-colors duration-200">
      <header className="sticky top-0 z-20 bg-white/80 dark:bg-gray-800/80 backdrop-blur-md shadow-sm p-4 transition-colors duration-200">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <button onClick={handleBack} className="px-6 py-2.5 bg-sky-600 text-white font-semibold rounded-full shadow-lg hover:bg-sky-700 transition-colors">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 custom-scrollbar pr-2" style={{ maxHeight: 'calc(100vh - 300px)', overflowY: 'auto' }}>
              {BACKGROUNDS.map(bg => (
                <div key={bg.id} className="group">
                    <div className="relative aspect-[3/4] bg-gray-200 rounded-xl shadow-md overflow-hidden border-2 border-transparent group-hover:border-sky-600 transition-all">
                        ...
                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center p-2">
                             <button
                                ...
                                className="w-full px-4 py-2 bg-white/90 text-sky-600 font-bold rounded-full shadow-lg text-sm hover:bg-white disabled:bg-gray-300 disabled:text-gray-500 disabled:cursor-wait"
                            >
          ...
          {(generatedImage || applyingBackgroundId || error) && (
            <section className="mt-8 pt-8 border-t border-gray-200 dark:border-gray-700">
              ...
              <div className="flex flex-col items-center">
                  {error && <div className="p-4 w-full max-w-lg bg-sky-100 dark:bg-sky-900/20 border border-sky-400 dark:border-sky-500 text-sky-700 dark:text-sky-400 rounded-lg text-center mb-4"><p>{error}</p></div>}
                  ...
                  {generatedImage && (
                    <div className="w-full max-w-lg animate-fade-in">
                        <div className="aspect-[4/5] bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden border border-gray-200 dark:border-gray-700 transition-colors duration-200 group relative">
                            <img 
                              src={generatedImage.imageUrl} 
                              alt="Generated with new background" 
                              className="w-full h-full object-contain cursor-pointer"
                              onClick={() => {
                                closedViaBackButton.current = false;
                                setFullscreenImage(generatedImage.imageUrl);
                              }}
                            />
                            <button
                              onClick={(e) => {
