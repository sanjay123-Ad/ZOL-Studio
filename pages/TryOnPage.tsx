...
import { UserIcon, GarmentIcon, DownloadIcon, SaveIcon, RefineIcon, ShareIcon, MaleIcon, FemaleIcon, AssetGeneratorIcon, ExpandIcon } from '../components/icons';
...
const TryOnPage: React.FC<TryOnPageProps> = ({ user, onSave }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const creditResult = await deductCredits(user.id, 1, 'virtual_tryon');
    ...
    <>
      ...
      <main className="w-full max-w-6xl bg-white dark:bg-gray-800 rounded-3xl shadow-2xl p-6 sm:p-10 border border-gray-200/50 dark:border-gray-700/50 transition-colors duration-200">
        <div className="text-center mb-2">
          <h1 className="text-display font-bold text-[#2E1E1E] dark:text-white font-headline">Virtual Photoshoot & Seamless Swap</h1>
        </div>
              ...
              <div className="flex flex-col items-center justify-center space-y-6">
                ...
                <div>
                    ...
                    <div className="inline-flex rounded-full bg-gray-100 dark:bg-gray-700 p-1 border border-gray-200/80 dark:border-gray-600/80">
                        {( [['1:1', 'Square'], ['4:5', 'Portrait'], ['9:16', 'Portrait Vertical']] as const).map(([value, label]) => (
                            <button
                                key={value}
                ...
                {error && (<div className="mt-4 p-4 bg-sky-100 border border-sky-400 text-sky-700 rounded-lg text-center"><p>{error}</p></div>)}
              </div>
            </Section>
          </div>
        )}
        ...
        {generatedImage && !isLoading && (
          <div className="mt-2 animate-fade-in">
            ...
            <div className="w-full flex justify-center">
              <div className="relative bg-gray-100 rounded-2xl shadow-lg overflow-hidden border-2 border-gray-200 p-2 max-w-lg w-full group">
                <img 
                  src={generatedImage} 
                  alt="AI Generated Photoshoot Result" 
                  className="rounded-xl w-full object-contain cursor-pointer"
                  onClick={() => {
                    closedViaBackButton.current = false;
            ...
            {error && (<div className="mt-4 p-3 bg-sky-100 border border-sky-400 text-sky-700 rounded-lg text-center text-sm"><p>{error}</p></div>)}
