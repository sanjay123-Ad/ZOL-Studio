...
const AssetGeneratorPage: React.FC<AssetGeneratorPageProps> = ({ user, onSaveToCollection }) => {
            ...
            if (savedState) {
                ...
                if (savedState.assetFiles && Array.isArray(savedState.assetFiles) && savedState.assetFiles.length > 0) {
                    ...
                    for (const af of savedState.assetFiles) {
                        // Load source image from IndexedDB (like Catalog Forge does)
    ...
    const handleBeforeUnload = async () => {
      ...
      if (generatedAssets.length > 0) {
        stateToSave = {
          ...
          assetFiles: assetFiles.length > 0 ? assetFiles.map(af => {
            ...
            return {
              ...
              imageFileKey: af.imageFile ? `asset-file-${af.id}` : null,
              status: af.status,
              error: af.error,
        ...
        for (const af of assetFiles) {
          if (af.imageFile) {
            await saveImage(`asset-file-${af.id}`, af.imageFile);
          }
        }
  ...
  }, [isStateRestored, view, gender, sourceImage, extractionScope, generatedAssets, savedAssetIds, garmentAnalysis, isBatchMode, assetFiles, batchSavedAssetIds, user.id, SOURCE_IMAGE_KEY]);
  ...
  useEffect(() => {
            ...
            if (generatedAssets.length > 0 || hasBatchResults) {
                ...
                stateToSave = {
                    ...
                    assetFiles: assetFiles.length > 0 ? assetFiles.map(af => {
                        ...
                        // Also save full generatedAssets for backward compatibility and as backup
                        const serializedAssets = (af.generatedAssets && Array.isArray(af.generatedAssets)) 
                            ? af.generatedAssets.map(ga => ({
                                category: ga.category,
                                itemName: ga.itemName,
                                views: (ga.views && Array.isArray(ga.views))
                                    ? ga.views.map(v => ({
                                        viewType: v.viewType || 'Front',
                                        imageBase64: v.imageBase64 || '',
                                    }))
                        ...
                        return {
                            ...
                            assetMetadata: assetMetadata,
                            generatedAssets: serializedAssets,
                            supabasePaths: af.supabasePaths || [],
                            sourceSupabasePath: af.sourceSupabasePath || undefined, // Save Supabase path for source
                            genderWarning: af.genderWarning || null,
                            isGenderMatch: af.isGenderMatch !== undefined ? af.isGenderMatch : true,
    ...
    saveCurrentState();
  }, [gender, sourceImage, extractionScope, view, generatedAssets, savedAssetIds, garmentAnalysis, isBatchMode, assetFiles, batchSavedAssetIds, user.id, SOURCE_IMAGE_KEY, isStateRestored]);
  ...
  const handleBatchFilesUpload = async (files: FileList | File[]) => {
    ...
    for (const assetFile of newFiles) {
      try {
        ...
        await saveImage(`asset-file-${assetFile.id}`, imageFile);
  ...
  const handleRemoveBatchFile = async (id: string) => {
    ...
    try {
      await deleteImage(`asset-file-${id}`);
    } catch (err) {
  ...
  const handleClearBatch = async () => {
    ...
    for (const file of assetFiles) {
      ...
      if (file.imageFile) {
        try {
          await deleteImage(`asset-file-${file.id}`);
        } catch (err) {
  ...
  const handleBatchGenerate = async () => {
    ...
    // Update only files that are NOT already successfully processed to analyzing status
    // This prevents reprocessing already completed files on reload
    setAssetFiles(prev => prev.map(f => 
    ...
    // First, analyze all files to determine how many credits will be needed
    ...
    try {
      // Analyze only files that need processing (in parallel for efficiency)
      const CONCURRENT_LIMIT = 2;
      for (let i = 0; i < filesToAnalyze.length; i += CONCURRENT_LIMIT) {
        const batch = filesToAnalyze.slice(i, i + CONCURRENT_LIMIT);
        await Promise.all(batch.map(async (assetFile) => {
          if (assetFile.imageFile) {
      ...
      // Process files in batches with proper concurrency (like Catalog Forge)
      for (let i = 0; i < filesToProcess.length; i += CONCURRENT_LIMIT) {
        // Filter out already processed files from this batch
        const batch = filesToProcess.slice(i, i + CONCURRENT_LIMIT).filter(f => !processedFileIds.has(f.id));
        
        if (batch.length === 0) continue; // Skip if all files in this batch are already processed
        
        // Process batch in parallel - all start together, wait for all to complete before next batch
        await Promise.all(batch.map(async (assetFile) => {
          ...
          try {
            ...
            setAssetFiles(prev => prev.map(f => 
              f.id === assetFile.id ? { ...f, status: 'processing', garmentAnalysis: analysis } : f
            ));
            ...
            if (assetFile.imageFile) {
              ...
              await deductCredits(user.id, creditsForThisFile, 'asset_generator');
                ...
                <div className="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700 animate-fade-in">
                    ...
                    <div className="flex flex-col md:flex-row md:space-x-6 space-y-3 md:space-y-0">
                                ...
                                {garmentAnalysis.canSeparate && garmentAnalysis.separationParts && garmentAnalysis.separationParts.length > 0 && (
                                    <label className={`flex items-center space-x-3 cursor-pointer p-3 rounded-lg border-2 transition-colors ${
                                        (extractionScope === 'separate' || extractionScope === 'multi-separate') 
                                            ? 'border-sky-600 dark:border-sky-500 bg-sky-50 dark:bg-sky-900/30' 
                                            : 'border-gray-300 dark:border-gray-600 hover:border-gray-400 dark:hover:border-gray-500'
                                    }`}>
                                        <input 
                    ...
                    {isProcessingBatch && batchProgress.total > 0 && (
                      <div className="mt-6 p-4 bg-sky-50 dark:bg-sky-900/20 rounded-lg">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-sm font-semibold text-gray-800 dark:text-white">Processing Batch</span>
                                  ...
                                  {file.generatedAssets.map((asset, idx) => {
                                    ...
                                    return (
                                      <div key={idx} className="relative group flex flex-col">
                                        <div className="relative aspect-[4/5] bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200 dark:border-gray-700">
                                          <img
                                            src={imageUrl}
                                            alt={asset.itemName}
