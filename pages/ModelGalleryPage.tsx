...
import { getModelsForGallery, Model } from '../services/models';
...
const ModelGalleryPage: React.FC<ModelGalleryPageProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    const garmentType = sessionStorage.getItem('model_gallery_garment_type') as 'upper' | 'lower' | null;
    if (galleryGender) {
      setFilteredModels(getModelsForGallery(galleryGender, garmentType || 'upper'));
    }
  }, []);
    ...
    <div className="min-h-screen flex flex-col bg-white dark:bg-gray-900 text-[#2E1E1E] dark:text-white transition-colors duration-200">
      <header className="sticky top-0 z-20 bg-white/80 dark:bg-gray-800/80 backdrop-blur-md shadow-sm p-4 transition-colors duration-200">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div>
            <h1 className="text-display font-bold font-headline text-[#2E1E1E] dark:text-white">The Model Studio</h1>
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              disabled={!selectedId}
              className="px-6 py-2.5 bg-sky-600 text-white font-semibold rounded-full shadow-lg hover:bg-sky-700 transition-colors disabled:bg-gray-400 dark:disabled:bg-gray-600 disabled:cursor-not-allowed"
            >
              Confirm & Apply
            </button>
          </div>
          ...
          {filteredModels.map(model => (
            <div key={model.id} className="flex flex-col">
              <button
                ...
                className={`relative group w-full aspect-[4/5] block border-4 rounded-2xl shadow-lg overflow-hidden transition-all duration-200 focus:outline-none ${selectedId === model.id ? 'border-sky-600 ring-4 ring-sky-600/30' : 'border-transparent hover:border-gray-400 dark:hover:border-gray-600'}`}
              >
                <img
                ...
                />
                 {selectedId === model.id && (
                  <div className="absolute inset-0 bg-black/30 flex items-center justify-center">
                    <div className="bg-sky-600 text-white rounded-full h-12 w-12 flex items-center justify-center ring-4 ring-white dark:ring-gray-800">
