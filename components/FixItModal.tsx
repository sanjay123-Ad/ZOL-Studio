      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col transition-colors duration-200" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit}>
            ...
            <div className="md:w-1/2 flex flex-col">
              ...
              <div className="space-y-3">
                {FIX_REASONS.map(reason => (
                  <label key={reason.id} className="flex items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg border-2 has-[:checked]:border-sky-600 has-[:checked]:bg-sky-600/5 dark:has-[:checked]:bg-sky-600/20 cursor-pointer transition-colors">
                    <input
                      type="radio"
                      name="fix-reason"
                      value={reason.id}
                      ...
                      className="h-4 w-4 text-sky-600 focus:ring-sky-600 border-gray-300 dark:border-gray-600"
                    />
              ...
              {selectedReason === 'other' && (
                <textarea
                  ...
                  className="mt-3 w-full h-24 p-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-md focus:ring-2 focus:ring-sky-600 focus:border-transparent animate-fade-in"
                  rows={3}
                  disabled={isRegenerating}
                />
              )}
               {error && <p className="text-sky-600 dark:text-sky-400 text-xs mt-2">{error}</p>}
            </div>
          </div>
          ...
          <footer className="flex justify-end p-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50 rounded-b-2xl gap-3">
            ...
            <button type="submit" disabled={isRegenerating || !selectedReason} className="px-6 py-2.5 bg-sky-600 text-white font-semibold rounded-full shadow-lg hover:bg-sky-700 transition-colors flex items-center justify-center disabled:bg-gray-400 disabled:cursor-not-allowed">
